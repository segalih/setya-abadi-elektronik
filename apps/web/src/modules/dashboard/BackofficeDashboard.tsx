...
export default function BackofficeDashboard() {
  ...
  const fetchData = async () => {
    try {
      const [healthResp, ordersResp, logsResp] = await Promise.all([
        api.get('/health').catch(() => ({ data: { status: 'down' } })),
        api.get('/backoffice/orders?limit=5'),
        api.get('/backoffice/audit-logs?limit=5')
      ]);
    ...
    <div className="space-y-10 pb-10 font-sans">
        ...
        <Card className="border-none shadow-sm rounded-2xl overflow-hidden flex flex-col">
          <CardHeader className="bg-slate-50/50 border-b flex flex-row items-center justify-between py-6">
            <div>
              ...
              <CardDescription className="text-xs">5 pesanan terbaru masuk sistem.</CardDescription>
            </div>
        ...
        <Card className="border-none shadow-sm rounded-2xl overflow-hidden flex flex-col">
          <CardHeader className="bg-slate-50/50 border-b flex flex-row items-center justify-between py-6">
            <div>
              ...
              <CardDescription className="text-xs">Aktivitas sistem & log audit terbaru.</CardDescription>
            </div>
            ...
            <CardContent className="p-8 text-center bg-pcb-grid bg-white min-h-[160px] flex flex-col items-center justify-center">
               {isOnline ? (
                 <>
                   ...
                   <h4 className="font-bold text-slate-800">System Balanced</h4>
      ...
      {isSupervisor && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
         <div onClick={() => navigate('/backoffice/audit')} className="p-8 rounded-[2.5rem] bg-gradient-to-br from-primary/10 to-transparent border border-white shadow-inner flex items-center gap-6 group hover:translate-y-[-4px] transition-all cursor-pointer">
         ...
         <div onClick={() => navigate('/backoffice/parameters')} className="p-8 rounded-[2.5rem] bg-gradient-to-br from-secondary/10 to-transparent border border-white shadow-inner flex items-center gap-6 group hover:translate-y-[-4px] transition-all cursor-pointer">
         ...
         <div className="p-8 rounded-[2.5rem] bg-gradient-to-br from-accent/10 to-transparent border border-white shadow-inner flex items-center gap-6 group hover:translate-y-[-4px] transition-all cursor-not-allowed opacity-60">
